...
vectorizer_lr = joblib.load(ML_MODELS_DIR / "LR_NFRA_vectorizer.pkl")
model_lr = joblib.load(ML_MODELS_DIR / "LR_NFRA_CLASSIFIER.pkl")
logger.info("Model 1 (LR) loaded")
vectorizer_rf = joblib.load(ML_MODELS_DIR / "RF_tfidf_vectorizer.pkl")
model_rf = joblib.load(ML_MODELS_DIR / "RF_CLASSIFIER.pkl")
logger.info("Model 2 (RF) loaded")


def classify_pages(pdf_path):
    logger.info(f"Opening PDF: {Path(pdf_path).name}")
    logger.info("Using ENSEMBLE mode (union of both models)")
    ...
    for page_num in range(len(doc)):
        ...
        pred_lr = normalize_category(model_lr.predict(vectorizer_lr.transform([cleaned]))[0])
        pred_rf = normalize_category(model_rf.predict(vectorizer_rf.transform([cleaned]))[0])
        
        for pred in [pred_lr, pred_rf]:
            if pred != "Others":
                category_pages.setdefault(pred, set()).add(page_num)
    
    doc.close()
    logger.info(f"Categories found: {list(category_pages.keys())}")
    return {k: sorted(v) for k, v in category_pages.items()}
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python category_segmentation.py <pdf_path> [--classify]")
